...
### Generate the maps tiles for the maps gallery

This command depends on data generated by the `extract_minimaps` and `extract_maps_data` commands.

  1. `pip install -r requirements-dev.txt`
  2. `set FLASK_APP=rwrs.py`
  3. `flask generate_maps_tiles`
